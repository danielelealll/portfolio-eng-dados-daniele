-- 1) A EQUIPE DE MARKETING PRECISA DE FAZER UMA PESQUISA SOBRE NOMES MAIS COMUNS DE SEUS CLIENTES E PRECISA DO NOME E SOBRENOME
...
--3) a equipe de produção de produtos precisa do nome de 
--todas as peças que pesam mais de 500 kg mas não mais que 700 kg para inspeção

SELECT *
from production.product
where Weight > 500 and weight <= 700;

-- 4) foi pedido pelo marketing uma relação de todos os empregados (employees) que são casados (single=solteiro, married = casado) 
...
-- um user chamado Peter Krebs está devendo um pagamento, consiga o email dele para que
...
-- eu preciso saber quantas pessoas tem o mesmo middlename agrupadas por middle name
...
-- eu preciso saber em media qual é a quantidade(quantity) que cada produto é vendido na loja
select productID, avg(OrderQty) as 'contaloja'
from sales.SalesOrderDetail
group by ProductID

-- eu quero saber qual foram as 10 vendas que no total tiveram os maiores 
--valores de venda (line total) por produto do maior 
-- valor para o menor
...
-- quantos produtos e qual a qtd media de produtos
...
-- estamos querendo identificar as provincias com o maior numero
--de cadastros no nosso sistema, entao é preciso encontrar quais provincias estão registradas no banco de
